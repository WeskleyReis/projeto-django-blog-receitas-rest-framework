...
class RecipeAPIv2TestMixin(RecipeMixing):
...
class RecipeAPIv2Test(test.APITestCase, RecipeAPIv2TestMixin):
    ...
    def test_recipe_api_list_loads_recipes_by_category_id(self):
        ...
        category_not_wanted = self.make_category(name='NOT_WANTED_CATEGORY')
         ...
         # Creates 10 recipes
        recipes = self.make_recipe_in_bath(qtd=10)
    ...
    def test_recipe_api_list_logged_user_can_update_a_recipe_owned_by_another_user(self):
        ...
        # This user cannot update the recipe because it is owned by another user
        ...
        # Assertion (Afirmação)
        # Another user cannot update the recipe,
        # so the status code must be 403 Forbidden
        self.assertEqual(
